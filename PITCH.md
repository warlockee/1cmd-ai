...
**onecmd** turns Telegram into a remote terminal controller.
...
```
You (phone)  -->  Telegram  -->  onecmd  -->  Your actual terminal
...
The optional AI manager turns onecmd from a remote control into an **autonomous terminal agent**.
...
Three converging trends create onecmd's opportunity:
...
**No one else does what onecmd does.**

| | onecmd | tgterm (antirez) | Shell bots | SSH apps | Warp / AI terminals | ChatOps (PagerDuty) |
|---|---|---|---|---|---|---|
...
**Closest competitor**: antirez's tgterm (macOS-only, no Linux, no AI, no security, no memory). onecmd is a fork that has diverged significantly.

**Key insight**: AI terminal tools (Warp, Claude Code, Cline) are **complementary** -- they run inside the terminals that onecmd controls. We're the remote access layer for the AI coding agent era.
...
```
Telegram  <-->  onecmd (C)  <-->  terminal sessions
                    |
                    |--- onecmd-ctl (C CLI)
                    |
...
- Open source on GitHub ([warlockee/1cmd-ai](https://github.com/warlockee/1cmd-ai))
...
- Establish onecmd as the standard for remote terminal access

### Phase 2: Managed Service
- **onecmd Cloud** -- hosted relay that eliminates self-hosting
...
- **API** -- third-party apps can control terminals via onecmd's infrastructure
...
| Infrastructure (onecmd Cloud) | 20% |
| Community & marketing | 15% |
| Legal & ops | 5% |

### 18-month milestones

1. **Month 3** -- onecmd Cloud beta (hosted relay, zero-setup install)
...
Building onecmd: experienced systems programmers with deep knowledge of macOS internals, terminal emulators, and LLM agent architectures.
...
*onecmd -- the missing remote control for the AI agent era.*

*Contact: [your-email] | GitHub: github.com/warlockee/1cmd-ai*
