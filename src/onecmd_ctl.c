...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd, "list") == 0) {
        ...
        if (argi + 1 >= argc) {
            fprintf(stderr, "Usage: onecmd-ctl capture <id>\n");
            return 1;
        }
        return cmd_capture(argv[argi + 1]);
    } else if (strcmp(cmd, "send") == 0) {
        if (argi + 2 >= argc) {
            fprintf(stderr, "Usage: onecmd-ctl send <id> <keys>\n");
            return 1;
        }
        return cmd_send(argv[argi + 1], argv[argi + 2]);
    } else if (strcmp(cmd, "status") == 0) {
        if (argi + 1 >= argc) {
            fprintf(stderr, "Usage: onecmd-ctl status <id>\n");
            return 1;
        }
