/*
 * backend_macos.c - macOS backend for onecmd
 *
...
static sds ax_read_value(AXUIElementRef element) {
    ...
    /* Replace null bytes (empty terminal cells) with spaces,
     * then trim trailing spaces on each line. */
    for (CFIndex i = 0; i < used; i++) {
        if (buf[i] == '\0') buf[i] = ' ';
    }

    sds result = sdsempty();
    CFIndex line_start = 0;
    for (CFIndex i = 0; i <= used; i++) {
        if (i == used || buf[i] == '\n') {
            /* Trim trailing spaces from this line. */
            CFIndex end = i;
            while (end > line_start && buf[end - 1] == ' ')
                end--;
            if (end > line_start)
                result = sdscatlen(result, buf + line_start, end - line_start);
            if (i < used)
                result = sdscatlen(result, "\n", 1);
            line_start = i + 1;
        }
    }
